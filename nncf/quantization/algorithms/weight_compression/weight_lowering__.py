...
def calculate_integer_quantization_params(
    ...
    if mode in [CompressWeightsMode.INT8_ASYM, CompressWeightsMode.INT4_ASYM]:
        ...
        scale, zero_point = calculate_scale_zero_point(
            ...
            level_high,
            narrow_range=False,
            invert_division=invert_division,
        )
...
def calculate_quantized_weight(
    ...
    ov_compression = (
        weight.backend in [TensorBackend.numpy, TensorBackend.ov] and is_openvino_available() and not NUMPY_COMPRESSION
    )
    ...
    if ov_compression:
        ...
        if not END_TO_END_COMPRESSION:
            ...
            compiled_model, compress_weight_primitive = (
                OV_COMPRESSION_PRIMITIVE_CACHE.get_compress_weight_primitive_end_to_end(
    ...
    if not ov_compression or COMPARE_WITH_NUMPY:
        ...
        if COMPARE_WITH_NUMPY and scale is None:
            ...
            if scale is None or zero_point is None:
                scale, zero_point = calculate_integer_quantization_params(
                    weight, reduction_axes, config, invert_division
                )
        ...
        if COMPARE_WITH_NUMPY:
            compare_np_to_ov(
                config, compressed_weights, compressed_weights_ov, scale, scale_ov, zero_point, zero_point_ov
            )
...
def do_int_quantization(
    ...
    compressed_weights, scale, zero_point = calculate_quantized_weight(
        weight, config, scale, zero_point, reduction_axes, invert_division
    )
    return compressed_weights, scale, zero_point
